...
def example_rotation_bfv():
    ...
    evaluator.rotate_rows_inplace(encrypted_matrix, 3, gal_keys, pool)
    plain_result = Plaintext()
    print("    + Noise budget after rotation: " +
          str(decryptor.invariant_noise_budget(encrypted_matrix)) + " bits")
    print("    + Decrypt and decode ...... Correct.")
    decryptor.decrypt(encrypted_matrix, plain_result)
    batch_encoder.decode(plain_result, pod_matrix, pool)
    print_matrix(pod_matrix, row_size)
    ...
    evaluator.rotate_columns_inplace(encrypted_matrix, gal_keys, pool)
    print("    + Noise budget after rotation: " +
          str(decryptor.invariant_noise_budget(encrypted_matrix)) + " bits")
    print("    + Decrypt and decode ...... Correct.")
    decryptor.decrypt(encrypted_matrix, plain_result)
    ...
    evaluator.rotate_rows_inplace(encrypted_matrix, -4, gal_keys, pool)
    print("    + Noise budget after rotation: " +
          str(decryptor.invariant_noise_budget(encrypted_matrix)) + " bits")
    print("    + Decrypt and decode ...... Correct.")
    decryptor.decrypt(encrypted_matrix, plain_result)
...
def example_rotation_ckks():
    ...
    # pool = MemoryPoolHandle().New(False)
    pool = MemoryManager.GetPool()
    ckks_encoder.encode(inputs, scale, plain, pool)
    ...
    evaluator.rotate_vector(encrypted, 2, gal_keys, rotated, pool)
    print("    + Decrypt and decode ...... Correct.")
    decryptor.decrypt(rotated, plain)
    result = DoubleVector()
    ckks_encoder.decode(plain, result, pool)
