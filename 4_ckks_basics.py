...
def example_ckks_basics():
    ...
    # public_key1 = keygen.public_key()
    # secret_key1 = keygen.secret_key()
    # relin_keys1 = keygen.relin_keys()

    # print('*'*100)
    # public_key1.save('public_key.bin')
    # secret_key1.save('secret_key.bin')
    # relin_keys1.save('relin_keys.bin')
    print('*'*100)
    public_key = PublicKey()
    secret_key = SecretKey()
    relin_keys = RelinKeys()
    public_key.load(context, 'public_key.bin')
    secret_key.load(context, 'secret_key.bin')
    relin_keys.load(context, 'relin_keys.bin')
    ...
    # x1_encrypted.save('cip.bin')
    # print("-" * 100)
    x1_loads = Ciphertext()
    x1_loads.load(context, 'cip.bin')

    x3_encrypted = Ciphertext()
    print("-" * 50)
    print("Compute x^2 and relinearize:")
    evaluator.square(x1_loads, x3_encrypted)
    ...
    evaluator.multiply_plain(x1_loads, plain_coeff3, x1_encrypted_coeff3)
    ...
    evaluator.multiply_plain_inplace(x1_loads, plain_coeff1)
    print("    + Scale of 0.4*x before rescale: " +
          "%.0f" % math.log(x1_loads.scale(), 2) + " bits")
    evaluator.rescale_to_next_inplace(x1_loads)
    ...
    print("    + Modulus chain index for x1_loads: " +
          str(context.get_context_data(x1_loads.parms_id()).chain_index()))
    print("    + Modulus chain index for x1_loads: " +
    ...
    print("    + Exact scale in  0.4*x: " + "%.10f" % x1_loads.scale())
    ...
    evaluator.mod_switch_to_inplace(x1_loads, last_parms_id)
    ...
    evaluator.add(x3_encrypted, x1_loads, encrypted_result)
